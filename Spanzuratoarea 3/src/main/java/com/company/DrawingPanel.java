...
public class DrawingPanel extends JPanel{
    ...
    public void playGame(String table) throws SQLException, IOException {
        ...
        player.setTabel(table);
        String word = player.generareCuvant();
        player.setCuvant(word);
        ...
        A.addActionListener(e -> {
            ...
            player.setLitera("a");
            List<Integer> lista =  player.cautareLitera();
            if (lista.size() == 0) {
                gresite++;
                switch (gresite) {
                    case 1:
                    ...
                    case 7:
                        try {
                            ...
                            long time =(stopTime - startTime)/1_000_000_000;
                            aiPierdut(word,time);
                        } catch (IOException ioException) {
                            ioException.printStackTrace();
                        }
    ...
    public void aiCastigat(String cuvant, long time) throws IOException {
        ...
        JLabel labelTime = new JLabel("<html><h1>Timpul petrecut: ");
        labelTime.setText(labelTime.getText()+ time + " secunde");
        label.setText(label.getText() + cuvant);
        frame.canvas.setLayout(null);

        frame.canvas.add(labelTime);
        frame.canvas.add(picLabel);
        frame.canvas.add(label);
        frame.canvas.add(label2);
        frame.canvas.add(newGame);
        frame.canvas.add(exit);
        labelTime.setBounds(340, 200, 500, 100);
    ...
    public void aiPierdut(String cuvant,long time) throws IOException {
